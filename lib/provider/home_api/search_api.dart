...
class SearchApi {
  ...
  Future<SearchApi> fetchAbout() async {
    final url = '${cons.Constants.apiUrl}/search?orderByAlphabet=true&query=بلوزة زرقاء اللون';

    final response = await http.get(url);

    if (response.statusCode == 200) {

      print("Searching is ${response.body}");

      return SearchApi.fromJson(jsonDecode(response.body));
    } else {
...
class Data {
  ...
  String name;
  String details;
  List<TranslationsProd> translations;
  ...
  Data.fromJson(Map<String, dynamic> json) {
    ...
    if (json['translations'] != null) {
      translations = new List<TranslationsProd>();
      json['translations'].forEach((v) {
        translations.add(new TranslationsProd.fromJson(v));
      });
    }
  }
...
class TranslationsProd {
  ...
  TranslationsProd.fromJson(Map<String, dynamic> json) {
    id = json['id'];
    name = json['name'];
    details = json['details'];
    productId = json['product_id'];
